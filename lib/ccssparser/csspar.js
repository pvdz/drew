...
var CssParser = (typeof module === 'undefined' ? {} : module).exports = (function(){
  var VERBOSE = false; // when false, suppresses logging
  // note: this` should be the global, and `window` wont exist in node, `require` wont exist in the browser (yet)

  var CssTokenizer = this.CssTokenizer || require('./csstok');
  var GssParser = this.GssParser || require('./gsspar');
  var Constants = this.Constants || require('./constants');

  var LOG = Constants.LOG;
  var WARN = Constants.WARN;
  var ERROR = Constants.ERROR;

  var TOKEN_NONE = Constants.TOKEN_NONE;
  var TOKEN_INCLUDES = Constants.TOKEN_INCLUDES;
  var TOKEN_DASHMATCH = Constants.TOKEN_DASHMATCH;
  var TOKEN_PREFIXMATCH = Constants.TOKEN_PREFIXMATCH;
  var TOKEN_SUFFIXMATCH = Constants.TOKEN_SUFFIXMATCH;
  var TOKEN_SUBSTRINGMATCH = Constants.TOKEN_SUBSTRINGMATCH;
  var TOKEN_IDENT = Constants.TOKEN_IDENT;
  var TOKEN_STRING = Constants.TOKEN_STRING;
  var TOKEN_FUNCTION = Constants.TOKEN_FUNCTION;
  var TOKEN_NUMBER = Constants.TOKEN_NUMBER;
  var TOKEN_HASH = Constants.TOKEN_HASH;
  var TOKEN_PLUS = Constants.TOKEN_PLUS;
  var TOKEN_GREATER = Constants.TOKEN_GREATER;
  var TOKEN_COMMA = Constants.TOKEN_COMMA;
  var TOKEN_TILDE = Constants.TOKEN_TILDE;
  var TOKEN_COLON = Constants.TOKEN_COLON;
  var TOKEN_AT = Constants.TOKEN_AT;
  var TOKEN_INVALID = Constants.TOKEN_INVALID;
  var TOKEN_PERCENTAGE = Constants.TOKEN_PERCENTAGE;
  var TOKEN_DASH = Constants.TOKEN_DASH;
  var TOKEN_CDO = Constants.TOKEN_CDO;
  var TOKEN_CDC = Constants.TOKEN_CDC;
  var TOKEN_WHITESPACE = Constants.TOKEN_WHITESPACE;
  var TOKEN_EOF = Constants.TOKEN_EOF;
  var TOKEN_ERROR = Constants.TOKEN_ERROR;
  var TOKEN_COMMENT = Constants.TOKEN_COMMENT;
  var TOKEN_NEWLINE = Constants.TOKEN_NEWLINE;
  var TOKEN_DOT = Constants.TOKEN_DOT;
  var TOKEN_PUNCTUATOR = Constants.TOKEN_PUNCTUATOR;
  var TOKEN_URL = Constants.TOKEN_URL;
  var TOKEN_BAD_URL = Constants.TOKEN_BAD_URL;
  ...
  // TOFIX: this is an ugly bridge to solve something that made sense in the old pattern but should be eliminated when possible
  var _nextFromLexer = function(){ throw new Error('I should have been overridden!'); };
  ...
  function parseGssRule() {
    ...
    try {
      var tree = GssParser(gssRuleTokens, $input);
    } catch (e) {
  ...
  function parseCssSelector(group, pseudo) {
    ...
    while (now !== $current) {
      ...
      switch (now.type) {
        ...
        default:
          return error('E_UNKNOWN_SELECTOR_TOKEN_TYPE: '+now.type);
      }
      start = false;
    }
          ...
          if ($current.value === '(') {
            ...
            if (name === 'calc') {
              ...
              if (rhpIndex) {
                ...
                try {
                  var calcArgTree = GssParser(calcTokens, $input);
                } catch (e) {
                  ERROR('GssParser threw error', e);
  ...
  function parseGssValue(propertyName) {
    ...
    // pass them on to GssParser
    // current will be the op, we dont pass that to GssParser either
    // after GssParser, combine them manually here.
    ...
    var tree = GssParser($cached, '<suppressed>');

    if ($current.value === ';') consume(); // skip semi's but keep eof and }

    // GssParser will encode `!require` data as a trailing pair `[..., require, !]`
  ...
  function parse(input) {
    ...
    _nextFromLexer = CssTokenizer.createTokenizer($input, $allTokens);
  ...
  var CssParser = {
    parse: parse,
    getAllTokens: function(){ return $allTokens; },
  };
