...
var CssTokenizer = (typeof module === 'undefined' ? {} : module).exports = (function(){
  var VERBOSE = false;
  ...
  function createToken(type, offset, len, value) {
    ...
    // TOFIX: regular expressions are relatively speaking super slow, we should benchmark a manual loop for this task
    if (type === TOKEN_STRING) value = value.replace(/\\(?:(?:\n\r)|[\n\r\f])/g, '');
    var url = '';
    if (type === TOKEN_URL || type === TOKEN_BAD_URL) {
      // slow but whatever, low frequency case
      url = value.slice(4, -1).replace(/^[\s\n]*(.*)[\s\n]*$/, '$1').replace(/^['"]|['"]$/g, '');
    }

    var t = {
      //BUILD_REMOVE_BEGIN
      _: Constants[type], // debugging
      //BUILD_REMOVE_END
      'value': value,
      'type': type,
      'offset': offset,
  ...
  function createTokenizer(input, allTokens) {
    $allTokens = allTokens;
    $input = input;
    $offset = 0;
    $row = 0;
    // row is tab-insensitive! so tab always counts as one char.
    // crlf also counts as one line, as per the standard.
    $lastNewlinePos = 0; // col=offset-lastNewlinePos
  ...
  CssTokenizer.createTokenizer = createTokenizer;
